...
// Helper function to get the next Thursday's date
function getNextThursday(): Date {
  const today = new Date();
  const dayOfWeek = today.getDay(); // 0 = Sunday, 1 = Monday, etc.
  
  // Calculate days until next Thursday (if today is Thursday, get next Thursday)
  const daysUntilThursday = dayOfWeek === 4 ? 7 : (4 + 7 - dayOfWeek) % 7;
  
  // If it's already Thursday and we want this Thursday instead of next, use this:
  // const daysUntilThursday = dayOfWeek === 4 ? 0 : (4 + 7 - dayOfWeek) % 7;
...
export default async function ThursdayPage() {
  // Get next Thursday's date in YYYY-MM-DD format
  const thursday = getNextThursday();
  const formattedDate = thursday.toISOString().split('T')[0];
  
  // Format the date for display
  const days = ['Søndag', 'Mandag', 'Tirsdag', 'Onsdag', 'Torsdag', 'Fredag', 'Lørdag'];
  const months = ['januar', 'februar', 'mars', 'april', 'mai', 'juni', 'juli', 'august', 'september', 'oktober', 'november', 'desember'];
  const dayName = days[thursday.getDay()];
  const day = thursday.getDate();
  const month = months[thursday.getMonth()];
  const formattedDisplayDate = `${dayName} ${day}. ${month}`;
  
  // Fetch fixtures for Thursday
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col md:flex-row-reverse gap-8">
        ...
        {/* Right column - Thursday's fixtures by league - Now first on mobile, second on desktop */}
