...
// Helper function to get the next Wednesday's date
function getNextWednesday(): Date {
  const today = new Date();
  const dayOfWeek = today.getDay(); // 0 = Sunday, 1 = Monday, 2 = Tuesday, 3 = Wednesday, etc.
  
  // Calculate days until next Wednesday (if today is Wednesday, get next Wednesday)
  const daysUntilWednesday = dayOfWeek === 3 ? 7 : (3 + 7 - dayOfWeek) % 7;
  
  // If it's already Wednesday and we want this Wednesday instead of next, use this:
  // const daysUntilWednesday = dayOfWeek === 3 ? 0 : (3 + 7 - dayOfWeek) % 7;
...
export default async function WednesdayPage() {
  // Get next Wednesday's date in YYYY-MM-DD format
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col md:flex-row-reverse gap-8">
        ...
        {/* Right column - Wednesday's fixtures by league - Now first on mobile, second on desktop */}
