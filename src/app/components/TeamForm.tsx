...
const TeamForm: React.FC<TeamFormProps> = ({ homeForm, awayForm, homeTeamId, awayTeamId }) => {
  const renderFormColumn = (form: Fixture[], perspectiveTeamId: number) => (
    <div className="space-y-3 flex-1">
      {form.length === 0 && <p className="text-sm text-gray-500 text-center">Ingen nylige kamper funnet.</p>}
      {form.map((fixture) => {
        ...
        return (
          <Link
            href={fixtureLink}
            key={fixture.id}
            className="flex items-center justify-between text-sm hover:bg-gray-50 p-1 rounded transition-colors duration-150"
          >
            <span className={`text-right ${fixture.teams.home.id === perspectiveTeamId ? 'font-medium' : ''} flex-1 truncate pr-2`}>
              {fixture.teams.home.name}
            </span>
            <span className={`px-2 py-0.5 rounded-md text-xs font-semibold ${result.className}`}>
              {result.text}
            </span>
            <span className={`text-left ${fixture.teams.away.id === perspectiveTeamId ? 'font-medium' : ''} flex-1 truncate pl-2`}>
  ...
  return (
    <div className="mt-6 p-4 md:p-6 bg-white rounded-lg shadow">
      <h3 className="text-lg font-semibold mb-4 text-center">Lag form</h3>
