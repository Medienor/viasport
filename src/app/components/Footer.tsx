      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              {internationalTeams1.map((team, index) => (
                <li key={index}>
                  <Link href={`/lag/${team.name.toLowerCase().replace(/\s+/g, '-')}-${team.id}`} className="hover:text-black">
                    {team.name} kamper
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              {leaguesAndChampionships.map((league, index) => (
                <li key={index}>
                  <Link href={`/fotball/liga/${league.name.toLowerCase().replace(/\s+/g, '-').replace(/å/g, 'a').replace(/ø/g, 'o').replace(/æ/g, 'ae')}-${league.id}`} className="hover:text-black">
                    {league.name}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-gray-300 mt-4 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-sm text-gray-600">© {currentYear} ViaSport. Alle rettigheter reservert.</p>
            <div className="flex space-x-6 mt-4 md:mt-0">
              <Link href="/om-oss" className="text-sm text-gray-600 hover:text-black">Om oss</Link>
              <Link href="/personvern" className="text-sm text-gray-600 hover:text-black">Personvern</Link>
              <Link href="/vilkar" className="text-sm text-gray-600 hover:text-black">Vilkår</Link>
              <Link href="/kontakt" className="text-sm text-gray-600 hover:text-black">Kontakt</Link>
            </div>
          </div>
        </div>
      </div>
