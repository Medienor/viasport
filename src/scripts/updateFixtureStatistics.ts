...
interface FixtureToUpdate {
  id: number;
  fixture_id: number; // The API fixture ID
}
...
async function updateFixtureStatistics() {
  console.log(colors.cyan('🔄 Starting Fixture Statistics Update...'));
  console.log(colors.gray('Fetching statistics for finished matches missing data\n'));

  // First, get count of all finished fixtures missing statistics
  console.log(colors.yellow('📋 Counting fixtures that need statistics...'));
  
  const { count: totalCount, error: countError } = await supabase
    .from('fixtures')
    .select('*', { count: 'exact', head: true })
    .eq('match_status', 'FT')
    .is('fixture_statistics', null);
  ...
  if (!totalCount || totalCount === 0) {
    console.log(colors.green('✅ No fixtures need statistics updates!'));
    return;
  }

  console.log(colors.cyan(`📊 Found ${totalCount.toLocaleString()} fixtures needing statistics`));
  ...
  while (currentPage * PAGE_SIZE < totalCount) {
    ...
    for (let i = 0; i < fixturesToUpdate.length; i++) {
      ...
      try {
        ...
        // Fetch statistics from API using the id column
        const statisticsResponse = await fetchFixtureStatistics(fixture.id);
        
        // Track empty responses
        if (Array.isArray(statisticsResponse) && statisticsResponse.length === 0) {
          emptyResponseCount++;
        }
        ...
        const { error: updateError } = await supabase
          .from('fixtures')
          .update({ fixture_statistics: statisticsResponse })
          .eq('id', fixture.id);
...
// Run the update
updateFixtureStatistics().catch(error => {
  console.error(colors.red('💥 FATAL: Unhandled error in updateFixtureStatistics:'), error.message);
  process.exit(1);
}); 